document.addEventListener("DOMContentLoaded", () => {
  ...
  // Definición de las URLs de referencia (las que sabemos que funcionan correctamente)
  const goodUrls = [
  ...
  // Función para extraer el nombre del producto de la URL (sin quitar la versión)
  function extractProductName(url) {
    // Se busca el segmento después de la versión y antes del primer delimitador (guion bajo, punto o guion)
    const match = url.match(/v\d+\/([^.]+)[._-]/);
    return match ? match[1].toLowerCase().replace(/[-_]/g, ' ') : null;
  }

  // Función para extraer la versión (ejemplo: v1742958440)
  function getVersion(url) {
    const match = url.match(/(v\d+)\//);
    return match ? match[1] : null;
  }

  // Función para calcular la similitud entre dos cadenas (de 0 a 1)
  ...
  // Función para comparar la nueva URL con las URLs buenas y corregir la versión si se detecta una alta similitud
  function fixUrl(newUrl) {
    ...
    // Si la similitud es mayor al 60%, usamos la versión de la URL buena
  ...
  function addMessage(text, type) {
    ...
    while ((match = markdownImageRegex.exec(text)) !== null) {
      ...
      // Aplica fixUrl para corregir la versión si hay coincidencia con las buenas
